...
import sys
'''
python train.py --model psp --backbone resnet50 --dataset sem --lr 0.001 --epochs 500 --workers 0 --batch-size 2
'''
...
def parse_args():
    ...
    parser.add_argument('--save-dir', default='~/.torch/fcn90_models',
...
class Trainer(object):
    ...
    def train(self):
        ...
        for iteration, (images, targets, _) in enumerate(self.train_loader):
            ...
            targets = targets.to(self.device)/255

            outputs = self.model(images)
            loss_dict = self.criterion(outputs, targets.long())
    ...
    def validation(self):
        ...
        for i, (image, target, filename) in enumerate(self.val_loader):
            ...
            target = target.to(self.device)/255

            with torch.no_grad():
                outputs = model(image)
                loss_dict = self.criterion(outputs, target.long())
            ...
            logger.info("Sample: {:d}, Validation pixAcc: {:.3f}, mIoU: {:.3f}, val_loss:{:.3f}".format(i + 1, pixAcc, mIoU, losses_reduced.item()))
